...
function RotatingFileStream(options) {
        ...
        for (let key in data) {
            if (internalFields.indexOf(key) === -1) {
                if (typeof(data[key]) === 'object') {
                    ...
                    output += util.format(" %s=\"%s\"", formatValue(key), formatValue(data[key].toString()));
                }
            }
        }
        return output;
    ...
    function write(s, callback) {
        if (typeof(s) === 'object') {
            let output = util.format('[%s] %s.%s L=%s E="%s" pid=%s', s.time.toISOString(), s.name, severityFromLevel(s.level), s.level, s.msg, s.pid);
